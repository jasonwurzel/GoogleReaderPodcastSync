...
namespace Tools.DnpExtensions
{
    ...
    public static class ObjectExtensions
    {
        ...
        public static T InvokeMethod<T>(this object obj, string methodName, params object[] parameters)
        {
            ...
            var method = type.GetMethod(methodName, Enumerable.Select(parameters, o=>o.GetType()).ToArray());
        ...
        {
            return Enumerable.Select((obj as Type ?? obj.GetType()).GetCustomAttributes(typeof(T), includeInherited).OfType<T>(), attribute => attribute);
        }
        ...
        static string CleanName(IEnumerable<char> name, bool isArray)
        {
            ...
            foreach (var c in Enumerable.Select(name.Where(c => Char.IsLetterOrDigit(c) && c != '`'), c => c))
                sb.Append(c);
            if (isArray)
                sb.Append("Array");
            return sb.ToString();
